...
from bot.keyboards.market import get_market_item_keyboard, get_inventory_item_keyboard
...
async def show_balance(callback: CallbackQuery, user: User, lang: str):
    ...
    await callback.message.edit_text(
        _("balance_info", lang, ton=user.balance_ton, stars=user.balance_stars, wallet=wallet_str)
    )
    await callback.answer()
...
async def show_sales(
    ...
    result = await session.execute(
        select(Listing, Gift).join(Gift).where(Listing.seller_id == user.id, Listing.is_active)
    )
    listings = result.all()
